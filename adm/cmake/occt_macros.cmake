...
function (OCCT_ORIGIN_AND_PATCHED_FILES RELATIVE_PATH SEARCH_TEMPLATE RESULT)
  ...
  file (GLOB ORIGIN_FILES "${OCC_ROOT_DIR}/${RELATIVE_PATH}/${SEARCH_TEMPLATE}")
...
macro (OCCT_INSTALL_FILE_OR_DIR BEING_INSTALLED_OBJECT DESTINATION_PATH)
  if (BUILD_PATCH AND EXISTS "${BUILD_PATCH}/${BEING_INSTALLED_OBJECT}")
    if (IS_DIRECTORY "${BUILD_PATCH}/${BEING_INSTALLED_OBJECT}")
      ...
      install (DIRECTORY "${OCC_ROOT_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
    ...
    if (IS_DIRECTORY "${OCC_ROOT_DIR}/${BEING_INSTALLED_OBJECT}")
      install (DIRECTORY "${OCC_ROOT_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
    else()
      install (FILES     "${OCC_ROOT_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
    endif()
  endif()
endmacro()
...
macro (OCCT_CONFIGURE_AND_INSTALL BEING_CONGIRUGED_FILE BUILD_NAME INSTALL_NAME DESTINATION_PATH)
  if (BUILD_PATCH AND EXISTS "${BUILD_PATCH}/${BEING_CONGIRUGED_FILE}")
    ...
    configure_file("${OCC_ROOT_DIR}/${BEING_CONGIRUGED_FILE}" "${BUILD_NAME}" @ONLY)
  endif()
...
macro (COLLECT_AND_INSTALL_OCCT_HEADER_FILES ROOT_TARGET_OCCT_DIR OCCT_BUILD_TOOLKITS OCCT_COLLECT_SOURCE_DIR OCCT_INSTALL_DIR_PREFIX)
  ...
  set (TEMPLATE_HEADER_PATH "${OCC_ROOT_DIR}/adm/templates/header.in")
  ...
  set (ROOT_OCCT_DIR ${OCC_ROOT_DIR})
...
macro (OCCT_COPY_FILE_OR_DIR BEING_COPIED_OBJECT DESTINATION_PATH)
  ...
  if (EXISTS "${OCC_ROOT_DIR}/${BEING_COPIED_OBJECT}")
    file (COPY "${OCC_ROOT_DIR}/${BEING_COPIED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
  endif()
...
macro (OCCT_CONFIGURE BEING_CONGIRUGED_FILE FINAL_NAME)
  if (BUILD_PATCH AND EXISTS "${BUILD_PATCH}/${BEING_CONGIRUGED_FILE}")
    ...
    configure_file("${OCC_ROOT_DIR}/${BEING_CONGIRUGED_FILE}" "${FINAL_NAME}" @ONLY)
  endif()
endmacro()
...
macro (OCCT_ADD_SUBDIRECTORY BEING_ADDED_DIRECTORY)
  ...
  elseif (EXISTS "${OCC_ROOT_DIR}/${BEING_ADDED_DIRECTORY}/CMakeLists.txt")
    add_subdirectory (${OCC_ROOT_DIR}/${BEING_ADDED_DIRECTORY})
  else()
...
function (OCCT_IS_PRODUCT_REQUIRED CSF_VAR_NAME USE_PRODUCT)
  ...
  if (NOT BUILD_TOOLKITS)
    ...
    foreach (USED_TOOLKIT ${BUILD_TOOLKITS})
      ...
      elseif (EXISTS "${OCC_ROOT_DIR}/src/${USED_TOOLKIT}/EXTERNLIB")
        file (READ "${OCC_ROOT_DIR}/src/${USED_TOOLKIT}/EXTERNLIB" FILE_CONTENT)
      endif()
...
function (FILE_TO_LIST FILE_NAME FILE_CONTENT)
  ...
  elseif (EXISTS "${OCC_ROOT_DIR}/${FILE_NAME}")
    file (STRINGS "${OCC_ROOT_DIR}/${FILE_NAME}" LOCAL_FILE_CONTENT)
  endif()
...
function (OCC_VERSION OCC_VERSION_MAJOR OCC_VERSION_MINOR OCC_VERSION_MAINTENANCE OCC_VERSION_DEVELOPMENT OCC_VERSION_STRING_EXT)
  ...
  set (STANDARD_VERSION_FILE "${OCC_ROOT_DIR}/src/Standard/Standard_Version.hxx")
